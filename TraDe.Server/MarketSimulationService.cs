...
// Flood the engine for Grafana metrics
...
public class MarketSimulationService : BackgroundService
{
    ...
    public MarketSimulationService(OrderProcessingChannel orderChannel, ILogger<MarketSimulationService> logger)
    {
        _orderChannel = orderChannel;
        _logger = logger;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await Task.Delay(5000, stoppingToken); // 5 sec warm-up to let you switch to Grafana
        _logger.LogInformation("STARTING HIGH-LOAD SIMULATION...");
        ...
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            // BATCHING: Send 200 orders every 10ms = ~20,000 orders/sec
            for (int i = 0; i < 100; i++) 
            {
                var change = (decimal)(_random.NextDouble() * 2 - 1) * 0.5m;
                _currentPrice = Math.Max(10, _currentPrice + change);

                var buyPrice = Math.Round(_currentPrice - 0.05m, 2);
                var sellPrice = Math.Round(_currentPrice + 0.05m, 2);
                var qty = _random.Next(1, 10); // Keep qty small for more matches

                // Create orders
                var buyOrder = new Order(buyPrice, qty, OrderSide.Buy);
                var sellOrder = new Order(sellPrice, qty, OrderSide.Sell);

                // Use TryWrite for maximum speed (fire and forget into the channel)
                // If the channel is full, skip (load shedding)
                _orderChannel.TryAddOrderAsync(buyOrder, stoppingToken); 
                _orderChannel.TryAddOrderAsync(sellOrder, stoppingToken);
            }
        }
    }
}