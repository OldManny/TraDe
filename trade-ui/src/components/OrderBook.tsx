  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl backdrop-blur-sm overflow-hidden">
      ...
      <div className="grid grid-cols-2 divide-x divide-slate-800">
        {/* Bids */}
        <div>
          <div className="grid grid-cols-3 gap-2 p-3 bg-slate-900/80 text-xs font-medium text-slate-400 border-b border-slate-800">
          ...
          <div className="p-2 space-y-1">
            {bids.map((bid, i) => (
              <div key={i} className="relative grid grid-cols-3 gap-2 p-2 rounded-lg hover:bg-emerald-500/5 transition-colors group">
                ...
                <div className="relative z-10 text-right font-mono text-slate-400 text-sm">{bid.total.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="p-2 space-y-1">
            {asks.map((ask, i) => (
              <div key={i} className="relative grid grid-cols-3 gap-2 p-2 rounded-lg hover:bg-rose-500/5 transition-colors group">
                ...
                <div className="relative z-10 text-right font-mono text-slate-400 text-sm">{ask.total.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
